...
void SceneGraph::RemoveLast(std::string node_name) { //finds and removes a scene node with a given name
	//node_.pop_back();
	std::vector<SceneNode*>::iterator position = std::find(node_.begin(), node_.end(), GetNode(node_name));
	if (position != node_.end()) {
		node_.erase(position);
	}
}
